  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      <nav className="mb-4 text-xs font-medium text-ink-500">
        ...
        <span className="mx-1">/</span>
        <span className="text-ink-900">{listing.name}</span>
      </nav>
        ...
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-2 border-b border-slate-200 pb-4">
            {['Overview', 'Services', 'Reviews', 'Location'].map((t) => (
              <span
                key={t}
                className="rounded-full bg-slate-100 px-4 py-1.5 text-xs font-bold text-ink-700"
              >
                {t}
              </span>
            ))}
          ...
          <section className="mt-6">
            ...
            <ul className="mt-4 grid gap-2 sm:grid-cols-2">
              {listing.highlights.map((h) => (
                <li
                  key={h}
                  className="flex items-center gap-2 rounded-xl border border-slate-100 bg-white px-3 py-2 text-sm font-medium text-ink-800"
                >
          ...
          <section className="mt-10">
            <h2 className="text-lg font-bold text-ink-900">Services &amp; prices</h2>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
            ...
            <div className="mt-6 border-t border-slate-100 pt-4">
              <p className="text-xs font-bold uppercase text-ink-500">From</p>
              <p className="text-2xl font-black text-ink-900">
                {formatRwf(listing.priceFromRwf)}
                <span className="text-sm font-semibold text-ink-500"> / visit</span>
              </p>
            </div>
            <Link
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-ink-600">
            <p className="font-bold text-ink-900">Guests love clarity</p>
            <p className="mt-2">
              “Feels like Booking.com but for my barber. I see the price and length
              before I commit.”
            </p>
            <p className="mt-2 text-xs font-semibold text-ink-500">— Pilot guest</p>
          </div>
        </aside>
      </div>
    </div>
